...
## ✅ 缺口⑥：彈藥庫E12/E14/E16/E18-E20共27張新卡，`fire_daily`延伸例句54句 — 已錄完接上（2026-08-12）

VERA用Colab腳本錄製54句mp3，已驗證全部存在、無0-byte、格式正確（MPEG layer III），
存進`audio/ammo/`並接進`AMMO_DAILY_AUDIO_MAP`（100→127筆）。以下清單留存當記錄。

**⚠️ 順手發現一個更早的既有缺口，尚未處理**：查證時發現E17自己的4張彈藥卡
（`e17_01`/`e17_02`/`e17_03`/`e17_09`，2026-07-24建的）從來沒有被排進錄音清單，
`AMMO_DAILY_AUDIO_MAP`裡完全沒有這4張的key，8句`fire_daily`延伸例句一直在fallback TTS，
不是這次造成的，是當初建卡時漏排。要不要一起排下一批待VERA決定。
