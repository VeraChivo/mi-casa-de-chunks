...
const GRAMMAR_DATA = [
...

  {
    ...
    family:{
      ...
      intro:"同一個「我想讓你知道我的判斷」骨架，因為想讓對方知道的事不同，會長出不同分支。<br><br>不是同義句堆疊，每句背後的溝通目的都不一樣。",
      items:[
...

  {
    ...
    family:{
      ...
      intro:"同一個「就要...了」骨架，因為想讓對方知道的事不同，會長出不同分支。<br><br>不是同義句堆疊，每句背後的溝通目的都不一樣。",
      items:[
  ...
  {
    id:"g42a", cat:"verb-pattern", level:"b1",
    title:"Quedar en + 動詞：約好做某事",
    rule:"quedar en + 動詞原形 → 約好做某事",
    examples:[
      {es:"Quedamos en vernos a las cinco.", zh:"我們約好五點見面。"},
      {es:"Quedé en llamarte esta noche.", zh:"我說好今晚會打給你。"}
    ],
    trap:"這個 quedar en 跟「quedarse（留下，反身動詞）」是不同用法，不要搞混。",
    source:"文法補充"
  },

  {
    id:"g42b", cat:"verb-pattern", level:"b1",
    title:"Quedar en que + 句子：講好……",
    rule:"quedar en que + 完整句子 → 講好某件事會發生",
    examples:[
      {es:"Quedamos en que él llamaría.", zh:"我們講好他會打電話來。"},
      {es:"Quedaron en que se verían el sábado.", zh:"他們講好星期六會見面。"}
    ],
    source:"文法補充"
  },
...

  {
    ...
    rule:"這兩句台詞都經多個獨立來源交叉比對確認過用字與出處，只收一句＋場景說明＋語氣解析，不重現完整對話——目標是「聽到這句話能聯想到哪部片、哪個場景、什麼語氣」，這是母語者看戲/聊電影時最自然會用到的文化默契。",
    examples:[
      ...
      {es:"¿Qué es un fantasma? Un evento terrible condenado a repetirse una y otra vez.", zh:"什麼是鬼魂？一件可怕的事，被判定要一遍又一遍地重演。（《El espinazo del diablo》，2001年，Guillermo del Toro導演，英譯片名The Devil's Backbone，字面「魔鬼的脊椎」。場景：全片開場與收尾的旁白，由Federico Luppi飾演的Casares醫生說出，前後呼應。語氣：低沉、像在下定義，把「鬼魂」講成一段停不下來的痛苦記憶，而不是恐怖片的怪物——這也是全片在西班牙內戰背景下真正想講的東西。💡condenado a + 動詞原形：被判定/註定要做某事，帶著無法擺脫的宿命語氣）"}
    ],
    trap:"看西語電影時，一句特別工整、聽起來像格言的台詞，常常同時在做兩件事：推進劇情，以及把一個抽象概念重新定義。像上面這句把fantasma（鬼魂）定義成「不斷重演的痛苦」，如果只抓字面意思（鬼＝恐怖片的怪物），就會完全漏掉導演真正在講的東西——記憶與創傷。聽到這種台詞時先停一下問自己「他是不是在重新定義一個我以為我懂的字」，這跟文學卡片裡讀Lorca/Borges需要的是同一種耳朵。",
    source:"文法補充"
  },
...

  {
    ...
    family:{
      title:"🌿 HACER：不只一種畫面",
      intro:"同一個「做/發生」骨架，因為想讓對方知道的事不同，會長出不同分支。<br><br>不是同義句堆疊，每句背後的溝通目的都不一樣。",
      items:[
...

  {
    ...
    family:{
      ...
      intro:"同一個「稱呼」骨架，因為想讓對方知道的事不同，會長出不同分支。<br><br>不是同義句堆疊，每句背後的溝通目的都不一樣。",
      items:[
...
/* CEFR 等級標籤（不當內容鎖，只當導覽用的視覺標籤＋篩選）
   2026-08-03修正：c1/c2原本借用了🌎拉美巡禮的分區名稱（🗣️街頭母語/🎭文化深度）當CEFR等級標籤，
   但這兩個名字是「內容類型」不是「難度等級」——點下去在💧文法儲水槽裡篩到的其實是g87/g88這類
   正式C1文法卡（新聞抽象動詞/正式連接詞），跟真正的街頭俚語/文化知識完全無關，c2甚至在儲水槽
   池子裡沒有任何一張卡符合（全部已分流去WORLD_ZONE_MAP），點了只會看到空清單。
   已改回單純的難度標籤，「街頭母語/文化深度」在儲水槽UI裡改成獨立的導覽捷徑(見
   renderGrammarSupplement)，不再假裝是這裡的篩選項目。*/
const GRAMMAR_LEVEL_TIERS = [
  {key:"a1a2", icon:"🌱", label:"護土嫩芽"},
  {key:"b1",   icon:"💧", label:"甘露超頻"},
  {key:"b2c1", icon:"🍯", label:"蜂王蜜釀"},
  {key:"c1",   icon:"🏆", label:"精通挑戰"},
  {key:"c2",   icon:"🌟", label:"巔峰造詣"}
];
/* 📊等級chip選定後，下方白話說明欄要顯示的文字（2026-08-03新增，VERA指定文案）——
   純輔助理解「這個等級在教什麼」，不是內容鎖，不影響篩選邏輯本身 */
const GSUP_LEVEL_INTRO = {
  all:   "從基礎到貼近母語，都在這裡",
  a1a2:  "打好句子骨架",
  b1:    "學會表達願望與假設",
  b2c1:  "釐清容易搞混的用法",
...
// 目的：避免未來看到「沒有句子」的卡就誤判成bug。一張卡沒有專屬劇情句可能是因為
// 它本來就是跨劇情概念卡（🔵），不是壞掉的孤兒。
// 🟢 active     現役能力：至少一句真實劇情/彈藥庫/媽媽語塊句子支撐核心用法
// 🔵 crossStory 跨劇情能力：本來就不綁單一劇情句，走💧文法儲水槽入口（source含「文法補充」）
// 🟡 historical 歷史能力：曾經有劇情句支撐，該劇情已被替換/移除，暫無現行入口（見g14案例）
// ⚪ incubating  待培養能力：目前保留給未來——新卡如果還沒接上任何真實入口，先標這裡，
...
// slang（🗣️街頭母語）＝口語/俚語/網路縮寫/口頭禪/固定搭配(locuciones)/易混淆地雷/語用詞，
// 核心判準：這是「母語者實際怎麼講話」，不是文化知識本身。
// culture（🎭文化深度）＝歷史/政治/文學/影視/禮儀/社會議題/飲食/節慶，
// 核心判準：這是需要理解的「文化知識/為什麼」，本身不是一句可以現學現用的口語表達。
// 未列在下面任一陣列的「文法補充」卡，維持留在💧文法儲水槽（純語言規則/句型/詞彙）。
const WORLD_ZONE_MAP = {
  slang: ['g24','g25','g26','g36','g37','g38','g39','g40','g46','g47','g59','g60','g68',
          'g81','g82','g83','g84','g85','g86','g89','g90','g91','g93','g100','g103','g110'],
  culture: ['g45','g48','g49','g61','g62','g63','g64','g65','g66','g67','g69','g70','g71',
            'g72','g73','g74','g78','g79','g80','g92','g94','g95','g96','g97','g98','g99',
            'g101','g102','g104','g105','g106','g107']
};

// ── 🌎拉美巡禮卡片收斂：次分類（原西語世界，2026-07-26更名；2026-07-25）──
// VERA要求「比照主頁：先顯示入口卡，點擊分類才展開」，不要點進🗣️/🎭就直接攤開26/32張。
// 純加法次分類，不改動WORLD_ZONE_MAP或任何卡片欄位——分類依「這句話實際在講什麼」手動歸類，
// 不是純粹依cat欄位機械分組（cat欄位太細碎，直接用會變成十幾個小分類，不符合「先做結構」的簡潔要求）。
const WORLD_SUBCAT = {
  slang: [
    {key:'idiom', icon:'🔤', label:'固定搭配', ids:['g36','g37','g38','g39','g40','g86','g90']},
    {key:'catchphrase', icon:'💬', label:'口頭禪', ids:['g24','g25','g26','g46','g47','g68']},
    {key:'internet', icon:'📱', label:'網路用語', ids:['g60']},
    {key:'slang', icon:'🗣️', label:'俚語', ids:['g59','g81','g82','g83','g84','g85','g89','g91','g93','g100','g103','g110']}
  ],
  culture: [
    {key:'history', icon:'📜', label:'歷史與社會', ids:['g95','g96','g97','g101','g102','g104','g105','g106']},
    {key:'life', icon:'🎉', label:'生活與節慶', ids:['g45','g48','g49','g61','g62','g63','g64','g65','g66','g67','g69','g71','g78','g79','g80']},
    {key:'people', icon:'👪', label:'人物與議題', ids:['g70','g72','g73','g74']},
    {key:'literature', icon:'📖', label:'文學影視', ids:['g92','g94','g98','g99','g107']}
  ]
};
