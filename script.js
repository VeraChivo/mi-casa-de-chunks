...
const SENTENCE_AMMO_MAP2 = {
  ...
  // E12/E14/E16 reusableChunk彈藥（2026-08-19，依CHUNK_ECOLOGY.reusableChunk清單，只建這15句，
  // episodeOnly/emotionalNarrative/culturalInfo不建ammo）
  112:['e12_03'], 113:['e12_04'], 119:['e12_10'],
  130:['e14_01'], 131:['e14_02'], 132:['e14_03'], 133:['e14_04'], 134:['e14_05'],
  135:['e14_06'], 136:['e14_07'], 137:['e14_08'], 138:['e14_09'],
...
// ══════════ 👅 口腔健舌坊（原🗣️西語純發音特區，2026-08-11改名）：第一輪R/RR（2026-08-10 VERA定案）══════════
...
const WORKSHOP_PROMPTS = [
  ...
  },
  {
    icon: '💬',
    label: '留言對練',
    tagline: '拿剛學會的語塊，去跟 AI 吵一架',
    why: '語塊背起來跟真的用出來是兩件事。這個是找 AI 演一個拉美網友，逼你把那個語塊實際講出口。',
    steps: [
      '先挑<b>一個</b>你最近學到、但還沒真的用過的語塊',
      '複製下面的 prompt，把 <b>[語塊]</b> 跟 <b>[主題]</b> 換成你要的',
      '貼進你慣用的 AI，它會先發一篇貼文引你回話',
      '用那個語塊回一則留言——講不順也照送，這裡沒有人在打分數',
      '它會用網友口吻回你，順便告訴你哪裡聽起來卡卡的'
    ],
    note: '它的角色是網友不是老師，只會說「這樣講有點生硬」，不會給你改考卷。想換語言一樣把「西語」「拉丁美洲」換掉就好。',
    prompt: [
      '你是一個拉丁美洲的網友，我在練西語，想找你對戲。',
      '',
      '請照這個順序來：',
      '',
      '1. 先用西語發一篇短短的社群貼文（3～4 句），主題是「[主題]」，語氣像一般人在發文抱怨或分享，不要像新聞稿。',
      '2. 貼文最後問我一句話，引我回應。',
      '3. 我會用西語留言回你。我這次想練的語塊是：[語塊]',
      '4. 你用網友的口吻回我的留言，繼續把話聊下去。',
      '',
      '回我的時候：',
      '• 先當一個真的網友回話，不要一開口就講解',
      '• 如果我那個語塊用對了，就自然接下去，不用特別誇獎',
      '• 如果哪裡聽起來不像母語者會講的，回完之後補一行小聲說「其實我們比較會說⋯⋯」，只挑最卡的一個地方講，不要一次列五個錯',
      '• 不要幫我打分數、不要給我評語等級',
      '',
      '規則：',
      '• 西語用拉丁美洲的講法（用 ustedes，不要用 vosotros）',
      '• 要附中文的話講人話，不要逐字對照的翻譯腔',
      '• 不要拿文法術語解釋（不要寫「虛擬式」「第三人稱」），直接說這句話在做什麼',
      '',
      '先發你的貼文，然後等我留言。'
    ].join('\n')
  }
    ...
    <div class="world-entry-list">
      <div class="world-entry-row" onclick="worldEntryJumpNews()">
        ...
        <div class="world-entry-body">
          ...
          <div class="world-entry-sub">用西語閱讀世界正在發生的事</div>
          <div class="world-entry-teaser">德國之聲 DW 西語新聞</div>
        </div>
      ...
      <div class="world-entry-row" onclick="worldEntryJumpSlang()">
        ...
        <div class="world-entry-body">
          ...
          <div class="world-entry-sub">聽懂西語圈真正使用的語言</div>
          <div class="world-entry-teaser">俚語・口頭禪・網路用語・固定搭配</div>
        </div>
        <span class="world-entry-arrow">→</span>
      </div>
      <div class="world-entry-row" onclick="worldEntryJumpCulture()">
        ...
        <div class="world-entry-body">
          ...
          <div class="world-entry-sub">探索西語世界背後的歷史、社會與文化脈絡</div>
          <div class="world-entry-teaser">歷史・節慶・人物・文學影視</div>
        </div>
        <span class="world-entry-arrow">→</span>
      </div>
    </div>
...
let _worldSlangSubcat = null;
let _worldCultureSubcat = null;
function _renderWorldZoneList(elId, zoneKey, selected, setFnName){
  const el = document.getElementById(elId);
  if(!el || typeof WORLD_SUBCAT === 'undefined') return;
  const subcats = WORLD_SUBCAT[zoneKey] || [];
  const chipsHtml = subcats.map(s => {
    const active = selected===s.key;
    return `<span class="gsup-level-chip${active?' active':''}" onclick="event.stopPropagation();${setFnName}('${active?'':s.key}')">${s.icon} ${s.label} (${s.ids.length})</span>`;
  }).join('');
  let listHtml = `<div class="garden-empty-msg">👆 選一個分類看看有哪些卡片</div>`;
  if(selected){
    const cat = subcats.find(s=>s.key===selected);
    const items = (cat?.ids||[]).map(id => GRAMMAR_DATA.find(g=>g.id===id)).filter(Boolean);
    listHtml = items.map(g=>{
      const lv = _gsupLevelInfo(g.level);
      return `
      <div class="gsup-row" onclick="openGrammarCard('${g.id}')">
        <div class="gsup-title-row">
  ...
  el.innerHTML = `<div class="gsup-level-filter" style="padding:8px 12px 4px">${chipsHtml}</div><div class="world-zone-items">${listHtml}</div>`;
}
function renderWorldSlangSection(){ _renderWorldZoneList('worldSlangBody', 'slang', _worldSlangSubcat, 'filterWorldSlangSubcat'); }
...
const WELCOME_TOUR_STEPS = [
  ...
  {icon:"🌱", title:"開始我的西語小日子", desc:"從第一句西語開始。<br><br>不管妳是第一次來，還是已經學過一點生活語塊，先選一個最貼近妳現在狀態的入口，我們直接帶妳過去：", levelButtons:true}
];
