...
## 4. 建議下一步（僅供參考，不代表本輪已執行）

這份清單本身不做任何修改。若要往下走，建議的優先順序：

1. **先處理「會誤導未來協作者」的項目**（1-2、2-6）——這兩項會讓下一個接手的人（不管是VERA自己還是
   其他AI）誤判某功能還沒做/誤判 header 長相，重工風險最高。
2. **再處理「文件內部就自相矛盾」的項目**（2-1）——不需要額外查證，直接對照就能修。
3. **1-3（clearLS範圍）、2-3（c1/c2死路chip）需要 VERA 先拍板**是保留現況、調整文案、還是調整程式碼，
   不是單純的文件同步，牽涉到產品判斷，比照 CLAUDE.md「AI修改權限」清單，這類判斷不该由AI自行決定。
4. **2-4（音檔缺口）** 可以直接生成一份 Colab 錄音腳本（比照工作守則第12條格式）補上這8句缺口，
   技術上單純，等 VERA 確認要不要排這輪。
5. **2-5（CHANGELOG.md停更）** 是否需要回補一週的摘要，或乾脆說明「CHANGELOG.md 已停用、CLAUDE.md
   才是唯一權威記錄」，屬於文件定位問題，建議直接問VERA要哪一種。
6. **1-1（ROADMAP.md 113→117）、2-7（措辭殘留）** 屬於最單純的文字更新，優先度最低，可以隨手批次處理。
